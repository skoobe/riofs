...
typedef void (*s3http_client_on_input_data_cb) (S3HttpClient *http, struct evbuffer *input_buf, gpointer ctx);
void s3http_client_set_input_data_cb (S3HttpClient *http,  s3http_client_on_input_data_cb on_input_data_cb);

// connection is closed
typedef void (*s3http_client_on_close_cb) (S3HttpClient *http, gpointer ctx);
void s3http_client_set_close_cb (S3HttpClient *http,  s3http_client_on_close_cb on_close_cb);

// connection is established
typedef void (*s3http_client_on_connection_cb) (S3HttpClient *http, gpointer ctx);
